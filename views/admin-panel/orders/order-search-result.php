...
<div class="col"> 
        ...
        <div class="row">
            ...
            <div class="col-md-auto align-self-center"> Orders <?php echo Session::has('updateOrderStatus') ? ' - ' . '<span class="alert alert-warning">' . Session::get('updateOrderStatus') . '</span>' : ''; ?> </div>
                ...
                <form action="<?php echo $config['url']['path'] . 'admin-panel/order-search-result'; ?>" class="col-md-5 input-group" method="post">
                    ...
                    <input id="name" type="text" class="form-control form-control-sm shadow-none" name="name" placeholder="Name, email, status, order number" required>
    ...
    <h3 class="font-weight-bold text-center">Search Result for '<?php echo !empty($_POST['name']) ? trim($_POST['name']) : ''; ?>'</h3>
                ...
                <tbody>
                    ...
                    <?php foreach($orders as $order) : ?>
                            ...
                            <tr>
                                ...
                                <td><a class="btn btn-primary btn-sm" href="<?php echo $config['url']['path'] . 'admin-panel/order-status/' . $order->customerID; ?>">Process Order</a></td>
